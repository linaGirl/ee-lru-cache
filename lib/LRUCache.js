    ...
    module.exports = new Class({
        ...
        , set: function(id, data, newTtl) {
            if (this._data[id]) {
                ...
                this._appendToList(this._removeFromList(id), null, newTtl);
            }
            else {
                // create new
                this._data[id] = { id: id, p: null, n: null, v: data };
                this._appendToList(this._data[id], null, newTtl);
                this._count++;
        ...
        // add to linked list
        , _appendToList: function(item, oldTtl, newTtl) {

            // set ttl flag
            item.t = (this._forceTtl && oldTtl) ? oldTtl : (Date.now() + (newTtl || this._ttl));
